...
const App = () => {
  ...
  const grandTotal = data
    ? data
        ...
        .map(rel => rel.map(t => Number(t.amount)).reduce((a, b) => a + b))
        .reduce((a, b) => a + b, 0)
        .toFixed(2)
    : 0
  ...
  return (
    <div className="container mx-auto">
      ...
      <div ref={printRef}>
        {data ? (
          <Relations relations={data} handleTypeChange={handleTypeChange} />
        ) : null}
        ...
        <div className="flex space-x-10">
          <div className="w-60 border border-black">
            ...
            <p className="bg-blue-200 flex justify-between border border-black">
              Restaurants: <span>${groupedAmountsByTypes[2]}</span>
            </p>
          ...
          <div>
            ...
            <p>
              Saldo Entre las Cuentas Restantes: <span>${groupToDivide}</span>
            </p>
            <p>
...
const Relations = ({ relations, handleTypeChange }) => {
  return (
    <div className="border-2">
      <h1 className="font-bold text-center bg-blue-200">CASHREWARDS (008)</h1>
          ...
          {trans
            ? trans.map((transaction, i) => (
                <tr
                  ...
                  className={`bg-${getColorByType(transaction.type)}`}
                >
                  {Object.keys(transaction).map(key =>
                    key !== "type" ? (
                      <td key={Math.random() * 1000} className="truncate">
                        {transaction[key]}
                      </td>
                    ) : null
                  )}
                  <td>
                    <select
                      ...
                      onChange={e =>
                        handleTypeChange(e.target.value, cardholder, i)
                      }
                    >
